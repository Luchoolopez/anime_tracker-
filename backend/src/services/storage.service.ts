...
export class StorageService {
   ...
   */
  async uploadProductImage(shopSlug: string, file: UploadedFile): Promise<string> {
  ...
  /**
   * Sube una imagen genérica a un folder.
   * Retorna url pública y key interna para borrar luego.
   */
  async uploadImage(folder: string, file: UploadedFile): Promise<{ url: string; key: string }> {
    const fileExtension = path.extname(file.originalname);
    const fileName = `${uuidv4()}${fileExtension}`;
    ...
    try {
      ...
      const publicBaseUrl = PUBLIC_ENDPOINT;
      return {
        url: `${publicBaseUrl}/${BUCKET_NAME}/${key}`,
        key,
      };
    } catch (error) {
      console.error('Error subiendo imagen a MinIO:', error);
  ...
  async deleteByKey(key: string): Promise<void> {
    try {
      ...
      console.error('Error borrando archivo en MinIO:', error);
    }
  }
